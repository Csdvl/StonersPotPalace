...
exports.decrementProductsQuantity = functions.firestore.document('orders/{id}').onCreate(cart => {
  ...
  // console.log(admin.firestore().collection('products').doc('2oRBmD2HwrJKSnvtEYdS').get().then(docRef => docRef.data.onStock));
  let onStock;
  pairs.map(pair => {
    admin.firestore().collection('products').doc(pair.id).get().then(docRef => {
      return onStock = docRef.data.onStock
    }).catch(err =>{console.log(err)});
    console.log(onStock);
    return admin.firestore().collection('products').doc(pair.id).update({ onStock: onStock - pair.quantity })
  });
